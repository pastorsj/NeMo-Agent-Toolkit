...
class NatEvaluator(NatBase[EvaluatorBaseConfig]):
    """Base class for evaluator configurations.

    Can be used in two ways:

    1. **Subclass pattern**: Use specific evaluator classes
       ```python
    ...
    2. **Factory pattern**: Pass a config object directly
       ...
       evaluator = NatEvaluator(config=config, name="my_evaluator")
       ```
