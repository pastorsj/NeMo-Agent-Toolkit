...
class LLMJudgeSelection(LLMJudgeSelectionConfig, NatTTCStrategy):
    """LLM Judge Selection Strategy"""

    judge_llm: LLMRef | typing.Any = Field(description="The LLM to use for the selection (judge) strategy.", init=False)
    nat_judge_llm: NatLLM = Field(description="The LLM to use for the selection (judge) strategy.", exclude=True)

    @model_validator(mode='after')
    def set_references(self):
        """Set judge llm name from llm object if llm is provided."""
        if self.nat_judge_llm:
            self.judge_llm = LLMRef(value=self.nat_judge_llm.computed_name)
        return self
